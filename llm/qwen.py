"""Tongyi Qianwen (通义千问) LLM provider."""
import json
import logging
import re
import time
...
class QwenProvider(AbstractLLMProvider):
    """Qwen API provider using OpenAI-compatible interface."""

    def __init__(self, config: dict):
        super().__init__(config)
        api_key = config.get("LLM_API_KEY", "")
        base_url = config.get("LLM_API_BASE", "https://dashscope.aliyuncs.com/compatible-mode/v1")
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if not response.choices:
                    raise ValueError("Qwen API returned empty choices")
                return response.choices[0].message.content or ""
            except Exception as e:
                if attempt == self.max_retries - 1:
                    raise RuntimeError(f"Qwen API failed: {e}")
                time.sleep(2 ** attempt)
        raise RuntimeError("Qwen API: unexpected retry loop exit")
    ...
    def embed(self, texts: list[str]) -> list[list[float]]:
        try:
            ...
            logging.getLogger("llm.qwen").warning(
        ...
        try:
            ...
            if match:
                try:
                    ...
                    pass
            raise ValueError(f"Failed to parse JSON from Qwen response: {raw[:500]}")
